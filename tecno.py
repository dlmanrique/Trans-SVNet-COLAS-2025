...
train_labels_80, train_num_each_80, train_start_vidx,\
    val_labels_80, val_num_each_80, val_start_vidx  = get_data(f'pkl_datasets_files/train_val_paths_labels_{args.dataset}.pkl')
...
num_videos_dataset = {'Autolaparo': [10, 4], #-> values for train and valid
                      'Cholec80': [40, 40],
                      'HeiChole': [16, 8],
                      'HeiCo': [21, 9],
                      'M2CAI': [27, 14]}
...
train_we_use_start_idx_80 = [x for x in range(num_videos_dataset[args.dataset][0])]
val_we_use_start_idx_80 = [x for x in range(num_videos_dataset[args.dataset][1])]

print("Train videos:", g_LFB_train.shape)
print("Valid videos:", g_LFB_val.shape)
...
for epoch in tqdm(range(args.max_epochs)):
    ...
    for i in tqdm(train_we_use_start_idx_80):
        ...
        wandb.log({"Train accuracy": batch_corrects_phase.data.item() / preds_phase.shape[0]})
    ...
    wandb.log({'F1_score': val_fscore_phase})
    ...
    if val_fscore_phase > best_val_fscore_phase:
        ...
        base_name = f"TeCNO50_best_model_epoch_{epoch}_f1_{val_fscore_phase}"

        # Save just the best model based on F1 score
        os.makedirs(f"TeCNO50_models/{args.dataset}", exist_ok=True)        
        torch.save(best_model_wts, f"TeCNO50_models/{args.dataset}/{base_name}.pth")
